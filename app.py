...
def load_chatbot(retrieval_mode="faiss"):
    import os
...
# Initialize session state for chatbot mode
if "retrieval_mode" not in st.session_state:
    st.session_state.retrieval_mode = "faiss"

# Load chatbot with selected mode
bot = load_chatbot(st.session_state.retrieval_mode)
...
with st.sidebar:
    ...
    mode_options = ["faiss", "hybrid"]
    selected_mode = st.radio(
        "Retrieval Mode",
        mode_options,
        index=mode_options.index(st.session_state.retrieval_mode),
        help="FAISS: Vector similarity | Hybrid: Supermemory hybrid retrieval"
    )

    if selected_mode != st.session_state.retrieval_mode:
    ...
    if st.session_state.retrieval_mode == "hybrid":
        st.info("🧠 **Hybrid Mode Active**\n\nUsing FAISS + Supermemory for enhanced retrieval with entity relationships and memory graph.")
